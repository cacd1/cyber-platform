...
import { X, Send, Sparkles, Bot, Paperclip, FileText } from 'lucide-react';
...
export const ChatNote = () => {
    ...
    return (
        <>
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/20 backdrop-blur-[2px]">
                    ...
                    >
                        ...
                        {/* Content Area */}
                        <div className={`h-[50vh] sm:h-[400px] max-h-[60vh] overflow-y-auto p-4 flex flex-col gap-3 custom-scrollbar ${styles.contentBg}`}>
                            {savedNotes.length === 0 ? (
                        ...
                        <form onSubmit={handleSave} className={`p-3 border-t flex gap-2 backdrop-blur-md ${styles.inputArea}`}>
                            <input
                                ...
                                className={`flex-1 border rounded-xl px-4 py-2.5 text-base focus:outline-none transition-all ${styles.input}`}
                                style={{ fontSize: '16px' }}
                            />
                            <input
                                type="file"
            ...
            <div className="fixed bottom-4 left-4 right-4 sm:left-6 sm:right-auto z-[90] flex flex-col items-start gap-3 pointer-events-none">
                <button
                    onClick={toggleOpen}
                    className={`
                        w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center transition-all duration-500 border border-white/10 pointer-events-auto
                        ${isOpen
